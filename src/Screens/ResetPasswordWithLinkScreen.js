...
import { resetPasswordWithLink } from "../Store/Actions/UserActions";
import { dontMatch, isEmpty } from "../utils/validation";
import { parse } from "query-string";
import { useLocation } from "react-router-dom";

const ResetPasswordWithLinkScreen = () => {
  const { search } = useLocation();
  const { secret, user } = parse(search);
  ...
  const resetPasswordHandler = async () => {
    ...
    try {
      const data = await dispatch(
        resetPasswordWithLink(secret, user, password)
      );
      if (data.error) {
        setError(data.error);
      } else {
    ...
    <div className="t-ctr flex1 fccc">
      ...
      {success && (
        <div className="p-text mt-3">
          Password Changed Successfully. You can login using new Password.
        </div>
      )}
    </div>
  );
...
export default ResetPasswordWithLinkScreen;
